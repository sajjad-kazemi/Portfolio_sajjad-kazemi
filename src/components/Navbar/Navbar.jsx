    ...
    <>
      <nav className={`${style.navbar} navbar-expand-md`}>
        ...
        <ul className={style.list}>
          <li><Link onClick={scrollUp} to='/'><ViewList/> { (window.innerWidth > 768) && 'Portfolio'}</Link></li>
          <li><Link onClick={scrollUp} to='/About'><Person/>{ (window.innerWidth > 768) && 'About'}</Link></li>
          <li><Link onClick={scrollUp} to='/Contact'><Envelope/> { (window.innerWidth > 768) && 'Contact'}</Link></li>
        </ul>
      </nav>
      <Outlet/>
    </>
